...
public final class Jaspic {
	...
	public static void logout(HttpServletRequest request, HttpServletResponse response) {
		try {
			request.logout();
			// Need to invalidate the session to really logout - request.logout only logs
			// the user out for the *current request*
			// This is nearly always unwanted. Although the SAM's cleanSubject method can
			// clear any session data too if needed,
	...
	public static void notifyContainerAboutLogin(Subject clientSubject, CallbackHandler handler,
			Principal callerPrincipal, Set<String> groups) {
		handleCallbacks(clientSubject, handler, new CallerPrincipalCallback(clientSubject, callerPrincipal), groups);
	}

	public static AuthStatus notifyContainerAboutLogin(Subject clientSubject, CallbackHandler handler,
			String callerPrincipalName, Set<String> groups) {
		handleCallbacks(clientSubject, handler, new CallerPrincipalCallback(clientSubject, callerPrincipalName),
				groups);
		return fromAuthenticationStatus(AuthenticationStatus.SUCCESS);
	}

	private static void handleCallbacks(Subject clientSubject, CallbackHandler handler,
			CallerPrincipalCallback callerPrincipalCallback, Set<String> groups) {
	...
	/**
	 * Should be called when the callback handler is used with the intention that an
	 * actual user is going to be authenticated (as opposed to using the handler for
	 * the "do nothing" protocol which uses the unauthenticated identity).
	 * 
	 * @param request
	 ...
	 * 
	 * @param context
	 *            the servlet context for which to obtain the JASPIC app context ID
	 ...
	 *            the server auth module to be registered
	 * @param servletContext
	 *            the context of the app for which the module is registered
		...
		String registrationId = AccessController.doPrivileged(new PrivilegedAction<String>() {
			public String run() {
				return AuthConfigFactory.getFactory().registerConfigProvider(
						new SamAuthConfigProvider(serverAuthModule), "HttpServlet", getAppContextID(servletContext),
	 ...
	 * 
	 * @param servletContext
	 *            the context of the app for which the module is deregistered
	 */
